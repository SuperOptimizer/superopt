...
def gen_random_expr(max_tokens, vars, constants):
  assert max_tokens > 0
  if max_tokens == 1:
    return f'({random.choice(vars + constants)})'
  elif max_tokens == 2:
    if random.random() < .5:
      op = random.choice(unary_ops)
      if op in ['++','--']:
        arg = random.choice(vars)
      else:
        arg = random.choice(vars + constants)
      return f'({op}({arg}))'
    else:
      op = random.choice(MATH_BINOPS)
      lhs = random.choice(vars + constants)
      rhs = random.choice(vars + constants)
      return f'(({lhs}){op}({rhs}))'
  elif max_tokens >= 10 and random.random() < .5:
  ...
  #elif max_tokens >= 6 and random.random() < .5:
    #statement expression
  #  return f"({{for(int _={random.choice(vars + constants)}   }})"
  elif max_tokens >= 3:
    op = random.choice(MATH_BINOPS)
...
def gen():
  ...
  vars = ['*' + x if random.random() < .5 else x for x in vars ]
  vars = random.sample(vars, random.randint(1,len(vars)))
  constants = [str(x) for x in random.sample(range(-1024,1024),len(vars))]
  dtypes = ['signed char', 'unsigned char', 'signed short', 'unsigned short', 'signed int','unsigned int','signed long long', 'unsigned long long']

  print(gen_random_func(max_tokens, vars, constants, dtypes, 'func'))
